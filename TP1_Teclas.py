...
def printInterfaz(palabra_a_adivinar,puntos,mensaje,letra_derrota,letras_usadas,intentos):
    """
...
def nuevaPartida(puntajeAnterior, palabraParaJuego):
    ...
    print("---------------")
    print("")
    print(f"Palabra a adivinar: {cadenaInterrogacion(palabra_a_adivinar, letras_usadas)} | Puntos: {puntos} | Letras incorrectas: {letras_derrota} | Intentos: {intentos}")
    print("")
    while not victoria(letras_usadas, palabra_a_adivinar) and letra_ingresada!="fin" and letra_ingresada!="0" and intentos !=0:
        letra_ingresada=inputLetra(letras_usadas)
        if darPuntos(letra_ingresada, palabra_a_adivinar):
            puntos += 10
            print("")
            print(printInterfaz(palabra_a_adivinar,puntos,MENSAJE_ACIERTO,letras_derrota,letras_usadas,intentos))
            ...
            print("")
            print(printInterfaz(palabra_a_adivinar,puntos,MENSAJE_DESACIERTO,letras_derrota,letras_usadas,intentos))
    if victoria(letras_usadas, palabra_a_adivinar):
        print("")
        print("¡Felicidades, has adivinado la palabra!")
    if intentos == 0:
        print("")
        print("No tienes mas intentos, vuelve a intentarlo")
        print("La palabra era", palabraParaJuego)
    if not exit:
    ...
    if seguir:
        ...
        print("")
        print("-------------------------------------------------")
        print("DESAROLLADORES: - Agustín Conti         [ETAPA 1]")
        print("                - Juan Ignacio D`Angona [ETAPA 2]")
        print("                - Maria Zahra           [ETAPA 3]")
        print("                - Agustín Yanuchausky   [ETAPA 4]")
        print("                - Micael Virgilio       [ETAPA 5]")
...
def ejecutarJuego(palabraParaJuego):
    ...
    print("     - Solo se permite una letra por ingreso.")
    print("     - Solo se permiten letras.")
    print("     - Tienes ocho intentos por cada palabra.")
    print("     - Para salir del juego ingrese: \"fin\" o \"0\".")
    print("---------------")
    print("")
    print(f"Palabra a adivinar: {cadenaInterrogacion(palabra_a_adivinar, letras_usadas)} | Puntos: {puntos} | Letras incorrectas: {letras_derrota} | Intentos: {intentos}")
...
def palabras_candidatas():
    ...
    print("")
    print("-----------------------------------")
    ver_lista = input("¿ Desea ver la lista de palabras disponible ?: ").upper()
    while ver_lista != "SI" and ver_lista != "NO":
        print("Ingreso inválido. Por favor, ingrese [\"SI\"/\"NO\"].")
        ver_lista = input("¿ Desea ver la lista de palabras disponible ?: ").upper()
    if ver_lista == "SI": 
